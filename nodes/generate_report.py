...
### 六、总结：一句话 + 三条要点
- 一句话总结公司当前经营状态（如"这家锂盐厂目前处于锂价下行期，但现金流充裕，能扛过去"）
- 3条可直接参考的关键结论
...
def _generate_fallback_report(data: Dict) -> str:
    ...
    # ── Chapter 6: Summary ──
    lines.append("## 六、总结：一句话 + 三条要点")
    lines.append("")

    # Generate summary from the data
    summary_line = _generate_summary_line(general, sector_inds, linkage, anomalies)
    lines.append(f"**一句话总结**：{summary_line}")
    lines.append("")

    lines.append("**三条要点**：")
    key_points = _generate_key_points(general, linkage, anomalies, skipped)
